...
1. HTTP(Hypertext Transfer Protocol): HTML을 전송하기 위한 통신 규칙(규약)
    ...
    - 통신규약: 데이터의 첫번째 줄에는 header 내용을 쓰고, 두번째 줄부터 html텍스트를 써서 주고받겠다는 규칙 같은 것.
...
4. SSL 디지털 인증서
    ...
    - SSL 통신에 사용할 공개키를 클라이언트에게 제공하여, 데이터를 암호화하여 통신할 수 있게 해준다.
...
3. 웹브라우저는 받은 SSL인증서를 (제3자에게) 확인하여, 자신이 접속하려는 서버가 맞는지 체크한다.

<br>

## SSL의 기능
1. 클라이언트가 접속하는 서버가 신뢰할 수 있는 서버인지 체크(내가 접속하려는 서버가 진짜 맞는지 등)
...
1. 클라이언트가 접속한 서버가 신뢰할 수 있는 서버임을 보장한다.
    - 접속한 서버가, 내가 접속하려고 한 서버가 맞는지 체크
2. SSL 통신에 사용할 공개키를 클라이언트에게 제공한다.
    - 인증서 안에 서버의 공개키가 들어가 있다.
    - 서버에 데이터 보낼 때, 공개키로 암호화해서 보내고,
    - 서버의 신원을 확인할 때, 공개키로 복호화해서 인증한다. 
...
1. 서비스 정보: 클라이언트가 접속한 서버가, 실제 접속하려는 서버가 맞는지 확인 용도
    - 인증서를 발급한 CA
    - 서비스의 도메인 등
1. 서버 측 공개키: 서버-클라이언트 간에 데이터를 암호화하여 통신하는 용도
    - 공개키의 내용
    - 공개키의 암호화 방법
...
2. 서비스(서버)측 공개키 (인증서에 기록)
    ...
    - 서버측 공개키를 이용해, '클라이언트-서버간 통신 데이터를 암호화' 한다.

<br><br>

1. CA: 인증서 > (CA비밀키)> 인증서 암호화 > 서버에 발급
1. 브라우저: 서버 접속
2. 서버: 암호화된 인증서 전송
2. 브라우저: (CA공개키)> 인증서 복호화 > 서버측 공개키 획득 >
3. 브라우저: (서버 공개키)> 데이터 암호화/전송 >
4. 서버: (서버측 비밀키)> 데이터 복호화
...
1. SSL 기능을 사용하기 시작하고, 브라우저 주소창에 자물쇠가 채워진다.
    ...
    - 서버 공개키를 통해, 클라이언트-서버간 데이터 전송을 암호화하여 수행
